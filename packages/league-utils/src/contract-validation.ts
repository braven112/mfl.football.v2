...
import type { ContractValidationResult, ContractValidationError } from '@mfl/shared-types';
...
/**
 * Validate league ID against allowed leagues
 * REFACTORED: Accepts allowedLeagues as parameter instead of hardcoding
 */
function validateLeagueId(leagueId: string, allowedLeagues: string[]): ContractValidationError[] {
  const errors: ContractValidationError[] = [];

  if (!allowedLeagues.includes(leagueId)) {
    errors.push({
      field: 'leagueId',
      message: `Contract management is only available for leagues: ${allowedLeagues.join(', ')}`,
    });
  }
 ...
 * REFACTORED: Accepts allowedLeagues parameter for flexibility
 */
export function validateContractSubmission(
  ...
  playerId: string,
  franchiseId: string,
  allowedLeagues: string[] = [], // Default to empty (no leagues allowed unless specified)
): ContractValidationResult {
  const errors: ContractValidationError[] = [];

  // Validate league (only if allowedLeagues specified)
  if (allowedLeagues.length > 0) {
    errors.push(...validateLeagueId(leagueId, allowedLeagues));
  }
