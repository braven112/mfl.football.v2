 ...
 */
export async function fetchLeagueData(options) {
    const { leagueId, year, outputDir, week = null, host = 'https://api.myfantasyleague.com', mflUserId, mflApiKey, force = false, } = options;
    ...
    const writeOut = (key, data) => {
        ...
        if (key === 'rosters') {
            ...
            if (!fs.existsSync(historyFile)) {
                ...
                fs.writeFileSync(historyFile, typeof data === 'string' ? data : JSON.stringify(data, null, 2), 'utf8');
    ...
    // Update metadata
    fs.writeFileSync(metaFile, JSON.stringify({ lastFetched: new Date().toISOString(), leagueId, year, week }, null, 2), 'utf8');
 ...
 */
export async function fetchFromEnv() {
