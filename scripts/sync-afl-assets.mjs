...
const ensureAliasForAsset = async (assetPath, franchiseId, bucket) => {
  ...
  try {
    ...
    console.warn(`[sync-afl] Missing source for ${bucket}: ${assetPath}`);
    return null;
  }
...
const aggregateAssets = async () => {
  ...
  try {
    ...
    console.warn('[sync-afl] Unable to read assets directory:', error);
  }
  ...
  for (const dirent of directories) {
    ...
    for (const file of files) {
      ...
      team.assets[folder].push({
        type: folder,
        filename: file,
        relativePath: `/assets/afl/${folder}/${file}`,
        extension: ext,
      });
    }
  }
    ...
    for (const entry of team.history ?? []) {
      ...
      for (const [bucket, assetPath] of assetPairs) {
        try {
          ...
          console.warn(`[sync-afl] Missing history asset: ${assetPath}`);
          continue;
        }
  ...
  const isDivisionBadge = (base) => /^(al|nl)-(?!champion)/.test(base);
  const readExtraDir = async (folder, classify) => {
    ...
    for (const file of files.sort()) {
      const ext = path.extname(file).toLowerCase();
      if (!allowedExtensions.has(ext)) continue;
      pushExtra(classify(path.basename(file, ext)), `/assets/afl/${folder}/${file}`);
    }
  };
  ...
  for (const dirent of directories) {
    ...
    if (!allowedExtensions.has(ext)) continue;
    pushExtra('league', `/assets/afl/${dirent.name}`);
  }
  try {
    await fs.access(resolvePublicPath('/assets/logos/afl-logo.svg'));
    extras.league.unshift({
      type: 'league',
      filename: path.basename('/assets/logos/afl-logo.svg'),
      relativePath: '/assets/logos/afl-logo.svg',
      extension: path.extname('/assets/logos/afl-logo.svg').toLowerCase(),
    });
  } catch {
    console.warn('[sync-afl] League logo not found: /assets/logos/afl-logo.svg');
  }
...
const run = async () => {
  ...
  console.log(
    `[sync-afl] Created/updated ${createdAliases.length} franchise aliases.`
  );
  console.log(
    `[sync-afl] Aggregated ${aggregatedTeams.length} teams into afl.assets.json.`
  );
};

run().catch((error) => {
  console.error('[sync-afl] Failed to create aliases:', error);
  process.exitCode = 1;
});
