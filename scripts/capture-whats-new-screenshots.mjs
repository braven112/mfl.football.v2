...
const MANUAL_CAPTURE_ONLY = {
  'afl-live-scoring-hero':
    "the entry links to /live-scoring (the full page), but the frame is the HOMEPAGE HERO — a blind capture of the link shoots the wrong feature entirely. Stage it by loading /afl-fantasy?testDate=2026-09-13, which puts the homepage in its Sunday game window, and taking an ELEMENT screenshot of `.lsh` rather than the page. Signed out on purpose: the conference scoping the entry describes needs an owner session, but a signed-out shot still shows the featured matchup, the compact grid and the LIVE badge without staging auth. Both themes are committed by toggling html.dark between the two shots.",
  ...
  'transaction-hub':
    'the hub is a MODAL opened by the header bell, not a page, so a blind capture of the entry link shoots Free Agents with nothing on top of it — and its two waiver screens are auth-gated on top of that. Stage it without a session: shoot the AFL, not TheLeague — the Waiver Priority row only renders where MFL reports a priority waiver system, and TheLeague is BBID_FCFS, so a TheLeague capture is missing the third row the entry describes. Load an AFL page with `?mockTrades=2` (which seeds the trade sections and auto-opens the trades list), click `#thm-list-back` to reach the hub home, rewrite the `#transaction-hub-config` blob with `signedIn: true`, `showWaiverPriority: true`, franchise 0001 and the American League team list (the hub script re-reads that blob on every call by design, so a DOM rewrite is enough), and route-stub /api/waiver-claims and /api/waiver-order — both 401 without an owner cookie, and an unstubbed order leaves the priority row with no rank chip.',
  'owners-poll':
  ...
  'afl-owners-poll':
    'same gate as owners-poll, on the AFL side: auth-gated, and the ballot only renders when a window is OPEN in Redis. The committed shot forges an AFL owner session for franchise 0009 — deliberately NOT the AFL admin franchise 0001, whose session renders the commissioner strip reading "No ballot open" over a ballot the stub says IS open — stubs GET /api/owners-poll/ballot with an open Week 1 window (slots 10, 9 of 24 in, closing Thursday 16:00 PT) and a filled ranking, and sets the Playwright context to America/Los_Angeles because the close time formats with toLocaleString(undefined) and a UTC container prints "Sunday, 5:05 PM UTC". Framed at viewport 1600x900 / deviceScaleFactor 1.6 to match the TheLeague capture\'s zoom, scrolled 120px so the title and card header both sit in frame.',
