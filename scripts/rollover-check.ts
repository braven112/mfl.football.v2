...
/** The six dates that matter for `year`: either side of Feb 14, either side of Labor Day, and mid-season. */
export function boundaryDates(year: number): Array<{ label: string; date: Date }> {
  const feb14 = new Date(Date.UTC(year, 1, 14, 20)); // 8:45 PT is 16:45 UTC; noon-ish UTC keeps the day unambiguous
...
async function main() {
  ...
  console.log('\nRead as pairs: league-clock pages change across Feb 14 only; season-clock pages change across Labor Day only.');
