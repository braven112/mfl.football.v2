/**
 * Ask Roger (TheLeague) system prompt — extracted from /api/rules-qa so the
 * eval harness (tests/eval/roger.eval.ts) exercises the EXACT production
 * prompt instead of a copy that drifts. The endpoint imports from here.
 *
 * Prompt-caching note: this string is the cache-eligible system block. Any
 * byte change invalidates the Anthropic prompt cache (one-time cost) — fine,
 * but don't inject per-request values here. The date block stays separate
 * (see rules-qa-handlers.ts#buildDateBlock).
 */

import { LEAGUE_CONSTITUTION } from './league-constitution';

/**
 * Every anchor Roger is allowed to link to. Must stay in sync with the
 * RULEBOOK SECTIONS list inside the prompt below —
 * tests/roger-eval-cases.test.ts enforces the sync.
 */
export const RULEBOOK_ANCHORS = [
  ...
  '#veteran-extensions',
  '#rookie-extensions',
  '#first-round-team-option',
  '#compensatory-picks',
  '#rookie-draft',
  '#free-agent-bidding',
  '#in-season-free-agent-blind-bidding-process',
  '#waiving-players',
  '#schedule',
  '#scoring-errors',
  '#game-tiebreakers',
...
/** Appended to the per-request date block — guards the "TODAY" hallucination bug. */
export const THELEAGUE_RULES_QA_DATE_SUFFIX =
  'Do not claim an event is "today" unless its calendar date matches the ISO date above.';

export const THELEAGUE_RULES_QA_SYSTEM_PROMPT = `You are "Roger" — the AI rules expert for The League, a 16-team dynasty salary cap fantasy football league established in 2007. You are NOT the Commissioner — you're Roger, a chatbot who's read the constitution cover to cover. Your answers are *probably* right, but for definitive rulings, owners should ask the actual Commissioner.
