...
- Doubleheaders must NOT fall on an NFL bye week. This is the league's TOP scheduling goal: fix the doubleheader weeks first and the rest of the schedule falls into place around them. If no league-wide week works in a given year, doubleheaders may be staggered franchise by franchise so each team plays its two games in a week its own roster is whole - a hand-built last resort, and still better than a doubleheader on a bye.
- Doubleheaders must be split between the start and the end of the season, and every franchise must get at least one doubleheader after Week 8. Because the NFL bye calendar moves every year, the doubleheader weeks are re-derived each season rather than fixed: recent seasons have used Weeks 1 and 2 early, plus whichever of Week 12 or Week 13 is bye-free that year.
...
- (Effective with the 2027 schedule; the 2026 schedule was drawn and locked before this rule was adopted.) The division games that CANNOT avoid a bye week are placed on the lightest bye weeks available - ideally ones with only two NFL teams out. Which division games land on a bye week is fixed by the format; which bye week they land on is not, and a two-team week costs a couple of rosters a starter where a six-team week guts half the league. This outranks the three-week rematch target: keeping rivals apart is ideal, but a schedule that protects the gap by handing a rivalry week to teams missing starters has bought the wrong thing. A bye week is only a real problem for a given game if one of those two teams is actually missing projected starters.
- (Effective with the 2027 schedule.) Division games are spread across the season: each franchise plays roughly half of its division games after the midpoint. This ranks just below keeping division games off bye weeks. The two genuinely conflict - bye-free weeks cluster early - and where they do, a division race that runs the whole season wins over a marginally cleaner one.
- (Effective with the 2027 schedule.) The final week of the regular season is a doubleheader whenever the calendar allows. This yields to the rule that no doubleheader may fall on a bye week; Week 14 has carried byes every season since 2021, so it has not been achievable in that time.
