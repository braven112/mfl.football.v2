...
export default function LiveBoard({
   ...
   */
  /**
   * The single panel's standings and leaders come from the panel AS DRAWN —
   * the held one when a read failed — so the tab, the table and the leaders
   * strip can never describe a different moment from the scores above them.
   */
  const soloPanel = panelViews.length === 1 ? panelViews[0].panel : null;
