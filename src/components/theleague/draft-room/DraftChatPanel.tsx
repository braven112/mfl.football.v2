...
const TENOR_API_KEY = 'AIzaSyDummy'; // Replace with real key via env var — falls back to no-GIF gracefully
const TENOR_LIMIT = 12;

async function searchTenor(query: string): Promise<TenorGif[]> {
  const key = (typeof import.meta !== 'undefined' && (import.meta as any).env?.PUBLIC_TENOR_API_KEY) || TENOR_API_KEY;
  if (!key || key === 'AIzaSyDummy') return [];

  try {
    const url = `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(query)}&key=${key}&limit=${TENOR_LIMIT}&media_filter=gif,tinygif`;
...
function GifPicker({ onSelect, onClose }: GifPickerProps) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      setLoading(true);
      const gifs = await searchTenor(query);
      setResults(gifs);
      setLoading(false);
    }, 350);
  ...
  const tenorConfigured = useMemo(() => {
    const key = (typeof import.meta !== 'undefined' && (import.meta as any).env?.PUBLIC_TENOR_API_KEY);
    return !!key;
  }, []);
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '0.375rem' }}>
        {!tenorConfigured ? (
          <div style={{ padding: '1rem', textAlign: 'center', fontSize: '0.75rem', color: 'var(--color-gray-400, #9ca3af)' }}>
            GIF search requires PUBLIC_TENOR_API_KEY
          </div>
        ) : loading ? (
          <div style={{ padding: '1rem', textAlign: 'center', fontSize: '0.75rem', color: 'var(--color-gray-400, #9ca3af)' }}>
            Searching…
