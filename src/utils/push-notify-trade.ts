...
/**
 * Per-league notification art (site-relative; every path must exist in
 * public/). One map rather than two ternaries so a league can never end up
 * with its own icon and the other league's badge.
 *
 * `icon` is the LARGE image the notification shows. `badge` is the SMALL
 * icon, and the two are not interchangeable: Android discards the badge's
 * RGB entirely and uses only its ALPHA channel as a stencil to tint, so an
 * opaque image renders as a solid filled block. That is exactly what a
 * missing badge used to produce — the service worker fell back to
 * TheLeague's `icon-192.png`, which is color type 2 with no alpha channel at
 * all, so the AFL's notifications showed a blank white square where its mark
 * belonged. The `badge-96.png` files are white-on-transparent silhouettes
 * generated by `scripts/generate-notification-icons.mjs`; regenerate them
 * there rather than hand-editing, and never point `badge` at a favicon.
 */
const LEAGUE_PUSH_ART: Record<string, { icon: string; badge: string }> = {
  afl: {
    icon: '/assets/afl/favicons/favicon-192.png',
    badge: '/assets/afl/favicons/badge-96.png',
  },
  theleague: {
    icon: '/assets/icons/pwa/icon-192.png',
    badge: '/assets/icons/pwa/badge-96.png',
  },
};

/** Per-league notification icon (site-relative, must exist in public/). */
export function leaguePushIcon(navSlug: string): string {
  return (LEAGUE_PUSH_ART[navSlug] ?? LEAGUE_PUSH_ART.theleague).icon;
}

/** Per-league notification badge — the Android small icon. See above. */
export function leaguePushBadge(navSlug: string): string {
  return (LEAGUE_PUSH_ART[navSlug] ?? LEAGUE_PUSH_ART.theleague).badge;
}
...
export function buildTradeOfferPayload(opts: {
  ...
  return {
    ...
    badge: leaguePushBadge(opts.navSlug),
  };
}
