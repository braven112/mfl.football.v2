...
/** Pure: what a weeklyResults payload says about one franchise's lineup for `week`. */
export function lineupSubmittedFromPayload(payload: any, week: number, franchiseId: string): boolean | null {
  const entry = findWeekResultsEntry(payload, week, { allowUnlabeled: true });
...
async function readLive(input: HasSubmittedLineupInput): Promise<any | null> {
  ...
  try {
    ...
    // `res.ok` is not "the call worked" — cache only a payload that carries the week.
    if (!findWeekResultsEntry(payload, week, { allowUnlabeled: true })) return null;
...
export async function hasSubmittedLineup(input: HasSubmittedLineupInput): Promise<boolean | null> {
  ...
  if (live) return lineupSubmittedFromPayload(live, input.week, input.franchiseId);
