...
export function buildPlayerCellHTML(opts: PlayerCellOptions): string {
  ...
  const avatarStyle = isLogo
    ? ''
    : ` style="--player-avatar-bg: ${getPlayerAvatarBackground(nflTeam ?? '')}; --player-avatar-border: ${getPlayerAvatarBorder(nflTeam ?? '')}; --player-avatar-ring: ${getPlayerAvatarRing(nflTeam ?? '')}; --player-avatar-ring-dark: ${getPlayerAvatarRingDark(nflTeam ?? '')}"`;
