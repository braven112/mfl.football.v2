...
- With 3+ leagues in the registry the chevron becomes a dropdown (`NavHeader`), automatic because `LEAGUE_PREFIXES` and the switch targets are now registry-derived. Its Escape handler runs in the **capture phase with `stopPropagation`** so closing the menu doesn't also close the drawer (NavDrawer has its own document-level Escape handler). To test 3-league mode, temporarily add a fake entry to `leagues-data.mjs` — the dev server picks it up without type errors blocking render (only `astro check` complains).
