...
- **The AFL builder is a different implementation.** `afl-fantasy/front-office/trade-builder.astro` is a server-rendered page with its own `picksFor()`. It doesn't use this React island, so a change to `DraftPickSelector`/`TeamPanel` affects TheLeague only.
