...
**Evidence:** `scripts/article-utils/article-links.mjs`, `scripts/schefter-weekly-articles.mjs` steps 8/10, `tests/article-type-interface.test.ts` (`requiredExports()` / `optionalExports()`), `tests/article-links.test.ts`.
