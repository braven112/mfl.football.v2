...
## 2026-09-10 - A Roster Is Three Lists, And Only One Of Them Has A Limit

**Context:** an AFL owner in the FCFS window tried to add a kicker and the modal
answered "Claim 1: your roster is full (16/16) — pick someone to drop." MFL said
otherwise: franchise 0005 held **14 ROSTER + 2 INJURED_RESERVE**, two active
slots genuinely open. The report arrived as "the AFL rosters seem to be
outdated", because that is what a wrong count looks like from outside — but the
read was live and current. It was the arithmetic that was wrong.

**Insight:** MFL's `export?TYPE=rosters` returns ONE `<player>` list per
franchise and separates the three buckets only by `status` — `ROSTER`,
`INJURED_RESERVE`, `TAXI_SQUAD`. Only the first consumes one of `rosterSize`'s
slots; the other two have their own limits on the league export (`injuredReserve`
is **10** in the AFL against a `rosterSize` of **16**, so the whole list can
legally reach 26). `MFLApiClient.getRosters()` flattened that list to bare ids,
and every consumer inherited a number that answers "who do I own" being used to
answer "how much room do I have". Those are different questions.

**Rule:** `getRosters()` is for OWNERSHIP ONLY — is this player taken, is that
player mine to drop, where an injured player counts exactly as much as a
starter. Anything measuring a roster AGAINST A LIMIT must use
`getRosterEntries()` and filter on `status` (`isActiveRosterStatus` /
`activeRosterIdsOf` in `src/utils/waiver-claim.ts`). An absent or unrecognised
status reads as ACTIVE — MFL omits the field for ordinary roster players in some
exports, and under-counting waves through an add that really does overfill.

**The mirror image, fixed in the same pass:** at 16 active + 1 IR, choosing the
IR player as the drop is not a legal add either — the add lands on the ACTIVE
roster, so the result is 17. MFL refuses it the way it refuses everything on
`add_drop`: HTTP 200, its own form re-rendered, nothing said. Caught up front
now with a message that names why.

**Where else this hides:** any place that compares a roster's `.length` to a
limit. The other write endpoints (`cut-player`, `move-to-ir`,
`move-to-practice`, `trades/submit`) only ever ask ownership questions of
`getRosters()`, which is why they were unaffected — but a new one that counts
slots must reach for `getRosterEntries()`.

Guards: `tests/waiver-claim.test.ts` — the unit rules plus a scan asserting the
