...
**Insight:** Every data file that references the shared sprite (`public/assets/icons/sprite.svg`) — `whats-new.json`, `page-directory.json`, `nav-config.json` — stores the bare glyph name; display code prepends `icon-`. A wrong value fails completely silently: `<use>` pointing at a missing fragment renders nothing. This class of bug is now blocked at PR time for `whats-new.json` and `page-directory.json`: `tests/helpers/sprite-icons.ts` exports `loadSpriteIconIds()` (parses the sprite's `id="icon-*"` symbols), and both data test files assert (1) no value starts with `icon-` and (2) every value exists in the sprite.

**Recommendation:** When adding an icon reference to any data file, use the bare glyph name and pick from the sprite's actual inventory (`grep -o 'id="icon-[^"]*"' public/assets/icons/sprite.svg`). If a new data file starts referencing the sprite, reuse `loadSpriteIconIds()` from `tests/helpers/sprite-icons.ts` in its data test rather than reimplementing the parse — `nav-config.json` is the known remaining unvalidated consumer.
