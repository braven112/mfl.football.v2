...
**Recommendation:** For any surface/text pair that must keep contrast across light and dark modes, take BOTH sides from the gray scale so they invert together — e.g. `background: var(--color-gray-800, #1f2937); color: var(--color-gray-50, #f9fafb)`. Never pair a gray-scale background with `--color-white` / `--color-black` text.
