...
**Recommendation:** Tables show a `<span class="na" title="{college}">-</span>` when a college has no logo — never the college name as text (row-height blowout, the thing the owner explicitly rejected) and never a bare `<img>` without `onerror="this.style.display='none'"` (broken icon; all four college-img sites now carry it). Text belongs on roomy surfaces only (PlayerDetailsModal already renders it). If a future refresh of college-logos.json regenerates entries, re-verify these four keys — a regeneration from the same name-matching source will resurrect all three bugs.
