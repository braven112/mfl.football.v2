...
**Evidence:** The fix needed two coordinated changes in the same mobile breakpoint: (1) in the component, `@media (max-width: 640px)` → `.view-selector { flex-wrap: nowrap; width: 100% }` + `.view-tab { flex: 1 1 0; min-width: 0 }`; and (2) in the page, `@media (max-width: 599px)` → `.standings-hero { justify-content: stretch }` and `.standings-hero__actions { width: 100% }` (and switching `.standings-hero__actions` from `display: grid` to `display: flex` so its child resolves `width: 100%` against the stretched width). Only with the ancestor stretched did `flex: 1` have room to work. Playwright shots at 320px and 360px with `html{font-size:19px}` (emulating large-font phones) showed all three tabs on one row. Both `theleague/standings.astro` and `afl-fantasy/standings.astro` embed the component and needed the same parent fix.

**Recommendation:** When `flex: 1`/`flex-grow` children won't expand, walk *up* the tree for a `justify-content: center` (or `justify-items: center`, or an `auto`/`max-content` grid track) — any shrink-to-fit ancestor starves the flex container of free space. Fix it by stretching the relevant ancestor (`justify-content: stretch` / `width: 100%`) in the same breakpoint, not by piling `width: 100%` on the flex container alone. Verify the narrowest target with a scaled-up root font — normal-font reasoning hides the wrap.
