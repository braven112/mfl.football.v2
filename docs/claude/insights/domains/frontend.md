...
for the best teams, red for the worst. Two helpers cooperate, and both took
`cat.direction`:

- `rankTeamsForColumn(values, direction)` sorts descending for an `'asc'`
  category and ascending for a `'desc'` one, so **rank 1 is already the BEST
  team** — the flag is fully consumed here.
- `getCellColor(rank, total, direction)` then branched on it again:
  `direction === 'asc' ? rank <= quartile : rank > total - quartile`.

For every `'desc'` category the two cancelled and the green quartile landed
back on the highest RAW value. Dead Money painted **$6.0M green and $213K
red**; same for Average Age, Expiring Contracts, Committed Salary %, Roster
Holes and Salary-Weighted Age — six of fourteen categories, on a page in the
Stats & Reports nav, telling every owner the opposite of the truth.

**Insight:** neither function is wrong on its own, and that is the whole
problem. Read either one in isolation and it looks like careful,
direction-aware code; the bug lives only in the composition. A reviewer
checking "does this handle both directions?" gets a yes from each half.

The generalisable rule: **once a value encodes an ordering, the flag that
produced it is spent.** A rank, a sorted array, a normalised score — anything
downstream consumes the ORDER, never the original direction. So the fix was to
delete the branch, not invert it, and to drop the parameter from
`getCellColor` entirely: a function that cannot see `direction` cannot
re-apply it, which is a stronger guard than any test asserting it doesn't.

**How to catch the next one:** when two helpers take the same flag, ask which
one *consumes* it. If the answer is "both", one of them is wrong. Grep the
...
## 2026-07-08 - `text-overflow: ellipsis` No-Ops on a `flex-end` (Shrink-Wrapped) Child — Needs `max-width`

**Context:** The Live Scoring matchup detail (`src/components/shared/LiveScoreboard.tsx` + `src/styles/live-scoring.css`) renders a head-to-head player row: the away player (`.ls-prow`) on the left, the home player (`.ls-prow.right`) mirrored on the right. Each name span `.ls-pname` already had `white-space: nowrap; overflow: hidden; text-overflow: ellipsis` and lived in a column flex `.ls-pid { min-width: 0; flex: 1 }`. On mobile the LEFT names truncated correctly, but the RIGHT (home) names rendered at full width and overlapped the score beside them (e.g. score "11.5" + name "Sam Darnold" painted as "11.5am Darnold").

**Insight:** The right column right-aligns its stack with `.ls-prow.right .ls-pid { align-items: flex-end }`. Any non-`stretch` `align-items`/`align-self` (`flex-end`, `center`, `flex-start`) makes the child **shrink-wrap to its content width** instead of filling the flex line — so `.ls-pname` sized to the full nowrap text and, being right-anchored, overflowed *leftward* over the sibling score. `text-overflow: ellipsis` never fired because the element's own width equalled its content, so there was nothing to clip. The left side only worked by accident: it used the default `align-items: stretch`, which forces the name to the container width, giving ellipsis a constrained box. `min-width: 0` on the flex parent is necessary (lets the parent shrink) but **not sufficient** — the shrink-wrapped child still overflows. Fix: `.ls-pname { max-width: 100% }` caps the shrink-wrapped box at the parent width so the clip happens.

**Evidence:** At 375px, before the fix the home name's left edge sat left of the score's right edge (measurable overlap); after adding `max-width: 100%` every home name cleared its score by 6px and ellipsized, with both teams' full lineups on screen and no horizontal page scroll. Measured via `getBoundingClientRect()` gaps + `scrollWidth > clientWidth`, not screenshots (per the 0×0-viewport caveat above). Same change also fixed a mobile head-to-head overflow: each side's non-shrinkable floor (position chip + headshot + score) exceeded the viewport, so the home column was pushed off-screen. The fix drops the *center* position label — not the per-side chips — because that single label is derived away-first (`a?.position || h?.position`) and so mislabels the home player whenever the two teams run different lineup shapes (a FLEX row can pair an RB against a WR); keeping each side's own (shrunk) chip labels every player correctly. With the center label `display: none` its DOM node collapses out of the grid, so `.ls-mx-row` drops from three tracks to two `minmax(0, 1fr)` side columns; that plus shrinking the headshot/score brings both rosters on screen.

**Recommendation:** Any ellipsized text inside a flex item aligned with a non-`stretch` value needs an explicit `max-width` (usually `100%`) — `overflow: hidden; text-overflow: ellipsis` silently no-ops otherwise because the box sizes to content. Pairs with the `minmax(0, 1fr)` grid insight below: shrinking the track lets the row fit; `max-width: 100%` makes the name actually clip within it.
...
## 2026-07-05 - Asset Library: League-Wide "Master" Downloads Belong in Config, Not the Scanned Folder

**Context:** Asked to surface a single league-wide master PSD (`AFL Team Logo.psd`, all teams' logos in one layered file) as a download on `/afl-fantasy/assets`. The obvious move — drop it into `public/assets/afl/psds/` alongside the per-team PSDs — is wrong.

**Insight:** `scripts/sync-{theleague,afl}-assets.mjs` builds `*.assets.json` by scanning each asset folder and assigning every file to a team by filename-slug (`teamMeta.get(fileSlug(basename))`). **Any file whose basename doesn't match a team meta becomes its OWN standalone card** via `registerTeam(slug, undefined)` — so a `afl_team_logo.psd` in the scanned folder mints a junk "Afl Team Logo" card that sorts into Active Teams next to real franchises. The generator *does* emit `extras` buckets (`championship`/`league`/`conference`/`division` — `aggregateAssets()` returns `{ teams, extras }`) and the page renders them (`League Logos`/etc.), but each extras entry is drawn as an `<img src={relativePath}>` preview served from `assetDomain` — fine for a lightweight rendered logo (png/svg), useless for an 87 MB binary `.psd` (no preview, wrong domain). Don't infer the extras schema from a checked-in `*.assets.json` either — it may still lack `extras` if it hasn't been regenerated since that feature landed.

**Recommendation:** Model league-wide master downloads as a `masterAssets` array in the league config (`afl.config.json`), each `{ label, description, filename, relativePath }`, and render a dedicated "Master Files" section in `assets.astro` that builds the href from `blobDomain + relativePath`. This (a) survives every `sync:*` regen because it's not folder-derived, (b) doesn't pollute the team grid, and (c) points straight at the Vercel Blob copy — the big `.psd` is gitignored (`*.psd`) and lives only in the blob, so the local file deliberately stays OUT of the scanned `psds/` folder. Upload the master file to the blob path that matches its `relativePath` (`assets/afl/psds/<name>.psd`) via a one-off `@vercel/blob` `put()` (or `pnpm upload:psds` if you also keep a local copy somewhere non-scanned). TheLeague's `live_standings_logos.psd` is the obvious next candidate for the same treatment.
...
## 2026-07-04 - Never Resolve `public/` Assets With `import.meta.url` in Astro Frontmatter

**Context:** `GET /afl-fantasy/icons` 500'd with ENOENT. The frontmatter read the icon sprite via `new URL('../../public/assets/icons/sprite.svg', import.meta.url)` — which resolved to `src/public/...` (nonexistent) because the relative walk starts at `src/pages/afl-fantasy/`, and `public/` lives at the repo root, one level above `src/`. The theleague twin of the page never broke because it used `path.join(process.cwd(), 'public/...')`.

**Insight:** `import.meta.url`-relative paths in `.astro` frontmatter are fragile twice over: (1) it's easy to miscount `../` segments since `public/` is a sibling of `src/`, not inside it, and (2) after `astro build`, compiled modules live in `dist/` chunks, so even a correctly-counted relative path can break in prerender/SSR. `process.cwd()` is stable in both dev and build for this repo (build always runs from the repo root).
...
## 2026-07-04 - Asset Files Contain Former Identities — Never Iterate `teams` Raw; Use getActiveTeams()

**Context:** `src/data/theleague.assets.json` holds 35 entries for 16 franchises: `category: 'active'` (current identity) plus 19 `category: 'former'` historical identities that REUSE the same franchise `id` (e.g. three entries with id `0011`). Former entries can also have a compound id (`"0002, 0013"` for the Sabertooths) or no id at all (Heavy Chevy). The AFL twin (`data/afl-fantasy/afl.assets.json`) is worse: 82 former vs 24 active. Note this is a different file from `theleague.config.json` — the year-resolution helpers (`getTeamIdentityForYear()`) don't apply here.

**Insight:** Every consumption pattern fails differently, and each looks fine for most ids — which is why this survived so long. `.map()` over the array duplicates UI per identity (the League Planner rendered 2-4 "Potential Targets" copies per team). `new Map(teams.map(t => [t.id, ...]))` is last-write-wins, so it silently resolves to a HISTORICAL name/icon wherever a former entry sorts later (Salary Analytics showed "Under Siege (2016–2018)" for 0011). The array sorts alphabetically by *name*, so `.find(t => t.id === ...)` is order-dependent and wrong for exactly the ids whose former name sorts before the active one (TheLeague: 0003, 0005, 0006, 0009, 0011, 0012 — this bit the Footer champion lookup). Random-pick (`teamsList[Math.floor(Math.random()*...)]`) can select a dead identity.

**Evidence:** First hit in `rosters.astro` (planner went from 34 fa-needs panels to exactly 16 after filtering). The full sweep landed as shared `getActiveTeams()` in `src/utils/league-assets.ts`, applied in salary.astro, calculator.astro, league-comparison.astro, rosters.astro (team list + banner plumbing), DraftCapitalTable.astro, Footer.astro (champion lookup), matchup-preview-example.astro, and team-preferences.ts (both leagues). Locked by `tests/league-assets.test.ts` (asserts exactly 16 unique active TheLeague ids, unique AFL ids, no era-suffixed names).

**Recommendation:** Any new consumer of either assets file MUST call `getActiveTeams(leagueAssets)` — never touch `.teams` directly. The only legitimate raw-array consumers are the asset-library pages (`theleague.astro`, `assets.astro` in both leagues), which intentionally show the full historical library and filter by category themselves. When a per-id lookup Map is built from this file, a wrong-but-plausible team name on screen is the symptom to watch for.

## 2026-07-05 - An ACTIVE Team's `assets.icons[]`/`banners[]` Is Oldest-First — `[0]` Is the Retired Logo

**Context:** Separate from the former-identity trap above (which is about extra top-level `teams` entries), each *single* active entry's `assets.icons` / `assets.banners` array folds the franchise's OWN former-identity art in at index 0, ordered **oldest-first**. Pigskins' `icons` is `["/history/pigskins_2007_icon_circle.png", "/icons/pigskins.png"]` — the retired 2007 circle logo precedes the live razorback. Only the four TheLeague franchises with a distinct former-identity icon are affected (Pigskins 0001, Bring The Pain 0008, Midwestside 0011, Dark Magicians 0015); every other team has a single-element array so `[0]` happens to be correct — which is why the bug looked like "only some teams."

**Insight:** Reaching for `team.assets?.icons?.[0]?.relativePath` renders the 2007-era logo for exactly those four teams. The roster page's `TeamIconNav` division switcher, Salary Analytics, the Cap Calculator, and the Footer's defending-champion medallion all did this. The asset-library page looked correct because it renders the *whole* array — only "pick one" consumers hit it.

**Evidence:** Fixed by adding `getCurrentIconPath()` / `getCurrentBannerPath()` (thin wrappers over `getCurrentAssetPath()`) to `src/utils/league-assets.ts` — they filter out any path containing `/history/` and take the newest remaining entry, falling back so single-element arrays still resolve. Applied in rosters.astro (line ~577, which plumbs through `teamsList` → `teamLookup` → draft cards), Footer.astro, salary.astro, calculator.astro. Verified live: all four nav icons resolved to `/icons/*.png` (200, 100×100).

**Recommendation:** To render a team's *current* logo/banner from an assets-file entry, call `getCurrentIconPath(team)` / `getCurrentBannerPath(team)` — never index `[0]`. `[0]` means "oldest," not "current," for any franchise that has ever rebranded its icon.

## 2026-07-05 - React Islands Must Never SSR Time-Derived Text — Null-Until-Mount + Reference-Clock Offset

**Context:** `TradeDeadlineHero.tsx` initialized its countdown state with `useState(() => new Date(targetISO).getTime() - Date.now())`. That initializer runs TWICE with different clocks: once during Astro SSR (where the page may have been resolved with `?testDate=`) and again at client hydration (real browser clock). Result: intermittent "Hydration failed because the server rendered text didn't match the client" — reproduced on ~2 of 6 loads of `/afl-fantasy?testDate=2026-11-18`, where the SSR tree said one thing and the client computed 3,265 hours remaining. Even without testDate, the seconds digit can differ between SSR and hydration on deadline day.

**Insight:** Two independent traps compound here. (1) Any `Date.now()`-derived text in an island's initial render is a hydration coin-flip — `client:idle`/`client:load` don't save you because React still diffs against the server HTML. (2) The repo's `?testDate=` convention makes it catastrophic rather than cosmetic: the server clock is the *reference* date while the client clock is *real* time, so the two trees can disagree about entire branches (expired vs. not), not just a digit.

**Evidence:** Fixed by making the countdown value `null` until `useEffect` runs — SSR and first client render both emit a stable `--:--:--` placeholder in the not-expired layout, so the trees are identical by construction. To keep test mode meaningful, both call sites pass `referenceNowISO` ONLY when the page resolved with `?testDate=` (TheLeague gates on `state.metadata.testMode`, AFL on `input.testMode` in the resolver); when present, the island ticks against that reference clock via a fixed offset measured at mount. Verified: 6 consecutive Playwright-driven loads with zero console errors, SSR HTML always contains the `--` placeholder, countdown ticks correctly from the testDate.

**Recommendation:** When an island renders anything derived from the current time (countdowns, "x minutes ago", live/expired branches), gate it behind mount: state starts `null`, first render emits a deterministic placeholder, the effect starts the clock. If the display should honor `?testDate=`, pass the page's reference instant as an ISO-string prop gated on an explicit `testMode` flag — do NOT infer test mode from client/server clock skew (Codex review caught this: a cached SSR response or delayed `client:idle` hydration exceeds any skew threshold and would silently lock a production countdown to a stale server clock). `suppressHydrationWarning` is the wrong tool here — it only silences single text nodes, not branch-level mismatches like expired vs. active layouts.

## 2026-07-06 - Headless Chromium Mirrors `backdrop-filter` Edges in Screenshots

**Context:** Capturing the trade-confirmation modal for a What's New image via Playwright (headless Chromium), the blurred page behind the modal overlay rendered with visibly mirrored text — "Pacific Pigskins" reading backwards — even though the same page looks correct in a real browser.

**Insight:** Headless Chromium's compositor samples `backdrop-filter` blur with edge-mirroring in captured frames, so any legible text behind a blurred overlay can appear reversed in screenshots. It's a capture artifact only — users never see it — but it makes promo/What's New screenshots look broken.

**Recommendation:** For screenshots that include a blurred overlay (the repo-wide `backdrop-filter: blur(2px)` modals), swap the blur for a visually-equivalent plain dim just for the shot: `overlay.style.backdropFilter = 'none'; overlay.style.background = 'rgba(15, 23, 42, 0.55)'` via `page.evaluate`, then capture. Don't change the real CSS — the blur rule is a repo requirement.

## 2026-07-07 - `flex: 1` Tabs Can't Fill a Row Inside a `justify-content: center` Grid Ancestor

**Context:** The standings view-switcher (`StandingsViewSelector.astro` — Division / Playoff / All-Play) used `flex-wrap: wrap`, so on phones (and with a scaled-up system font) the third tab dropped to a second row. The obvious fix — `flex-wrap: nowrap` + `.view-tab { flex: 1 1 0; min-width: 0 }` so the tabs share one row — did nothing on its own, because the tabs still collapsed to their content width instead of expanding to fill the row.

**Insight:** `flex: 1` only distributes *available* free space, and there's no free space unless the flex container actually stretches to a definite width. On the standings page the selector sits inside two nested `display: grid; justify-content: center` wrappers (`.standings-hero` → `.standings-hero__actions`). `justify-content: center` sizes each track to its content (shrink-to-fit) rather than stretching it, so the flex container inherited a content-width, `flex: 1` saw zero free space, and the tabs stayed at min-content. Setting `width: 100%` on the flex container is circular/inert inside a shrink-to-fit grid track — the track is still content-sized. You have to make an ancestor *stretch* first.

**Evidence:** The fix needed two coordinated changes in the same mobile breakpoint: (1) in the component, `@media (max-width: 640px)` → `.view-selector { flex-wrap: nowrap; width: 100% }` + `.view-tab { flex: 1 1 0; min-width: 0 }`; and (2) in the page, `@media (max-width: 599px)` → `.standings-hero { grid-template-columns: 1fr }` and `.standings-hero__actions { width: 100% }` (and switching `.standings-hero__actions` from `display: grid` to `display: flex` so its child resolves `width: 100%` against the stretched width). The `599px` boundary is deliberate — the hero's own two-column rule is `min-width: 600px`, so below it the hero is a single implicit column; forcing that column to `1fr` makes it full-width (cleaner and more spec-explicit than `justify-content: stretch`, which only stretches *auto* grid tracks). Only with the ancestor column full-width did `flex: 1` have room to work. Playwright shots at 320px and 360px with `html{font-size:19px}` (emulating large-font phones) showed all three tabs on one row. Both `theleague/standings.astro` and `afl-fantasy/standings.astro` embed the component and needed the same parent fix.

**Recommendation:** When `flex: 1`/`flex-grow` children won't expand, walk *up* the tree for a `justify-content: center` (or `justify-items: center`, or an `auto`/`max-content` grid track) — any shrink-to-fit ancestor starves the flex container of free space. Fix it by giving the relevant ancestor a definite full width in the same breakpoint (`grid-template-columns: 1fr` for a single-column grid, or `width: 100%` on a stretchable block), not by piling `width: 100%` on the flex container alone. Verify the narrowest target with a scaled-up root font — normal-font reasoning hides the wrap.

## 2026-07-07 - `max-width: 0` Collapses a `display: flex` Table Cell to Nothing Under Auto Layout

**Context:** Making the AFL conference playoff-standings card (`src/components/afl/hp-sections/AflConferencePlayoffPreview.astro`) fit on phones. The team-name `<td>` is `display: flex` (name span + optional "Div" chip). To truncate long names as a safety net I set `.afl-conf__td--team { max-width: 0; }` plus `overflow: hidden; text-overflow: ellipsis` on the inner span. The screenshot came back with the entire Team column blank — every name truncated to zero width — while Seed/Record/PF rendered fine.

**Insight:** In a table with default `table-layout: auto`, the browser sizes columns from content, but an explicit `max-width` on a cell is honored as a hard cap. When that cap is `0` and the cell's content can overflow-hide (here the flex span had `min-width: 0`, so it *could* shrink to nothing), the column collapses to zero and the text vanishes — the other columns (right-aligned Record/PF) happily absorb the freed space. The fix that actually addressed the overflow was structural (hiding the redundant Div column on mobile via `display: none`), which freed enough width that the team names fit with no cap at all. The ellipsis-on-the-span rule is a harmless no-op once the cell is allowed to size to content.
...
## 2026-08-16 - `college-logos.json` Can Point at the Wrong School Entirely — and `logo: null` Is the Sanctioned "No ESPN Art" State

**Context:** The Free Agents table rendered a broken-image icon plus full-width alt text ("Malone University") for Ashton Dulin, doubling his row height. Root cause was two distinct data defects in `src/data/college-logos.json`: (1) tiny schools ESPN has no logo art for at all (Malone `556`, Manitoba `2770` — both cuts 404 permanently), and (2) a name-based ESPN-id mismap: "Louisiana" / "Louisiana-Lafayette" pointed at id `2347`, which is **Louisiana Christian University** (also no art), while the Ragin' Cajuns are id `309`. The mismap had shipped a dead logo for every UL product (e.g. Elijah Mitchell) since the file was built.
