...
**Evidence:** Fixed by making the countdown value `null` until `useEffect` runs — SSR and first client render both emit a stable `--:--:--` placeholder in the not-expired layout, so the trees are identical by construction. To keep test mode meaningful, both resolvers now pass `referenceNowISO` (TheLeague via `state.metadata.referenceDate`, AFL via a new field on the trade-deadline state variant); at mount the island measures skew between `Date.now()` and the reference, and if it exceeds 60s (test mode) it ticks against the reference clock. Verified: 6 consecutive Playwright-driven loads with zero console errors, SSR HTML always contains the `--` placeholder, countdown ticks correctly from the testDate.

**Recommendation:** When an island renders anything derived from the current time (countdowns, "x minutes ago", live/expired branches), gate it behind mount: state starts `null`, first render emits a deterministic placeholder, the effect starts the clock. If the display should honor `?testDate=`, pass the page's reference instant as an ISO-string prop and apply the skew-threshold offset (small skew = hydration latency, use real clock; large skew = test mode, use reference clock). `suppressHydrationWarning` is the wrong tool here — it only silences single text nodes, not branch-level mismatches like expired vs. active layouts.
