...
## 2026-07-07 - Shared Astro Partials: Behavior in the Partial, Layout in the Parent (display: contents)

**Context:** FeatureCompositeHero and AflEventHero each carried ~90 duplicated lines of browser-frame screenshot markup + CSS (chrome bar, light/dark theme-pair swap, 404 fallback classes, lazy→eager promotion script, mobile logo stand-in). The implementations had already drifted (FCH honored a forced-dark class in its promotion script; AFL didn't). Extracted into `src/components/BrowserFrameShot.astro`.

**Insight:** The split that makes a shared partial work across differently-laid-out parents: the partial's root renders `display: contents` and owns behavior (state classes, theme swaps, onerror fallbacks, inline scripts) plus whatever placement is genuinely identical across parents (right-flank anchor, vertical centering, tilt); each parent overrides only the values that truly differ (`right`, `width`) via `<scoped-parent> :global(.partial__part)` rules, appearance flows through CSS custom properties (`--bfs-shadow`, `--bfs-logo-opacity(-dark)`) set on the hero root, and per-league constants (logo pair, domains) come from the league registry, not a map in the partial. `display: contents` means the partial's children position against the parent's `position: relative` ancestor exactly as inline markup would — no wrapper box to fight — while the wrapper element still works as a `closest()` target for onerror state classes and still applies/inherits custom properties. Two supporting tricks: (1) encode conditional CSS states with `:not()` guards (`.bfs--dark:not(.bfs--no-dark)`) instead of override-rule ordering — source-order dependencies across a scoped stylesheet are fragile; (2) boolean render-time modes (forced dark) travel as a prop that sets a modifier class on the partial root, so both CSS selectors and the inline script can key off it without knowing the parent's class names.

**Recommendation:** When two components duplicate a visual treatment but position it differently, extract a `display: contents` partial that owns markup + state + swap logic + the shared placement base, leaving only the genuinely-different layout values in the parents. Note: `HeroBanner.astro` and the What's New listing pages still carry their own older single-image `.browser-frame` CSS (no theme pair) — candidates for migrating onto BrowserFrameShot if they ever grow dark captures.
