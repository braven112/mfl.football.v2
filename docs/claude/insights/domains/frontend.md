...
**Evidence:** Converting `<main class="franchise-detail">` → `<div class="franchise-detail">` and deleting `max-width: 960px; margin: 0 auto` let the page fill the layout's 1232px container (hero banner full-width, trophy grid +1 badge per row). Before/after Playwright shots confirmed the only visual change. A bare-`main`-selector grep across all 35 pages returned zero CSS targeting the element, so a blanket `<main>`→`<div>` (keeping the class) was safe everywhere — 71 insertions / 73 deletions, the 2 extra deletions being the trophy wall's width rule. The other 34 pages were pixel-identical before/after (the swap is visually inert).
