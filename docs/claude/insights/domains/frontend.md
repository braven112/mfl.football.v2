...
**Insight:** These two components are deliberate visual twins but share **no CSS** — each has its own scoped `<style>` with a parallel-but-duplicated class tree (`afl-conf__*` vs `afl-tiers__*`). A mobile fix applied to one does not propagate. When touching the mobile header layout of either, mirror the change into the other or they visibly diverge stacked on the same screen. Note one deliberate difference in the mirrored block: `.afl-tiers__td--team` is a plain `<td>` (not `display: flex` like `.afl-conf__td--team`), so its name-truncation safety net uses `display: inline-block; max-width: 100%` on the span — NOT the `max-width: 0` trap documented in the 2026-07-07 conf-card insight, which zeroes an auto-layout column.
