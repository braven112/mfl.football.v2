...
- After re-pulling history, a recompute run WITH `--emit-milestone-posts` (the backfill workflow passes it; plain `pnpm compute:franchise-history` no longer writes the feed since Sept 2026) emits retroactive Schefter milestone posts for "new" badges — revert `src/data/theleague/schefter-feed.json` if the badges are just corrected history, not news. Dedupe is snapshot-based (`franchise-history.json`), so the revert doesn't re-emit later.
...
**Last three art gaps closed from Brandon's personal files (2026-07-04):** **Mistakes Were Made** (0003, 2007–09), **Acer FC Edge** (0007, 2007–09), and **DangerZone** (0013, 2007–08) had NO surviving art anywhere online — exhaustively verified: the Wayback Machine's *complete* inventory of `dynastytheleague.com/images/*` is a single 2011-02-08 crawl of the 16 icons live that day (all three teams were renamed by 2010, so their files were unreferenced and never captured), and exact-URL CDX queries (both hostname variants, any status), `theleague.us`/`afl-fantasy.com` archives, `mfladdons.com` listings, live-host probes, and web mirror searches were all empty. Brandon then found the **original source PSDs** (900×150 full banners — larger than anything the Wayback had) plus 100×100 circle icons in his Downloads folder. Flattened via `sips -s format png` and wired into config as `{slug}_banner.png` + `{slug}_icon_circle.png`. Brandon's stash also included the **complete original 100×100 logo-button set for every identity** — all 18 legacy `*_icon_circle.png` files were replaced in place with the authentic buttons (previously they were circles improvised by cropping the 300×50 mini-banners). Most were byte-for-byte filename-compatible overwrites (no config change); Drunk Indians moved from `drunk_indians_afl_icon.png` (deleted) to `drunk_indians_icon_circle.png`, and both Midwestside Connection history entries (0010 2011–15, 0011 2019–24) moved from the modern active icon to the vintage `midwestside_icon_circle.png`. Poker in the Rear's button matches the 2012–13 art; the 2015 era keeps its distinct mfladdons-derived crop. With these, **every historical identity in both leagues has authentic art** — the only feed identities not in either config are AFL 2009's two "(open team)" placeholder slots, which is intentional. If art ever looks missing again, check owner's personal files early; the web has nothing pre-2011 for this league.

**Second batch (same day):** full-size banner PSDs restored The Executioners, Rolling Rockers, Amish Rakefighters, and BOYZ II MEN banners (config `banner` upgraded from the 300×50 minis), and three same-name era entries were added for teams whose art changed without a rename: **Pacific Pigskins 2007–2012** (black razorback), **Bring The Pain 2007–2022** (skull & crossbones), **Dark Magicians of Chaos 2007–2022** (classic wordmark) — files named `{slug}_2007_banner.png` / `{slug}_2007_icon_circle.png` (year-suffix convention from `poker_in_the_rear_2015_*`). Same-name history entries fold into the active team's Asset Library card (PR #310 fold logic) while still driving era-correct standings art via `getTeamIdentityForYear`; include `nameMedium`/`nameShort`/`abbrev` in them or historical contexts lose the short names. All source PSDs live in `public/assets/theleague/history/psd/` and are linked as downloads from the `vintage-art-restored` What's New entry. The defending-champion medallion on standings is era-correct now too (standings.astro prefers era identity; falls back to current only for placeholder icons). Third batch (same day) delivered full banners for 11 of those plus AFL Minnesota Road Kill (in-place overwrite of the Wayback mini). Still missing: full banners for **LBer-DeCleaters** and **Devil Dogs**; a logo button for **Generals (2014)**; the 2007–09 team pages (850×420) for everyone but Acer.
...
**Recommendation:** `scripts/fetch-adp.mjs` now sends `IS_KEEPER=N` / `IS_KEEPER=D` and warns loudly (`::warning::`) when both payloads fingerprint identical — treat that warning as "the filter regressed again," not noise. Related endpoint worth knowing: `TYPE=aav` returns average AUCTION values with the same filter params — arguably the better value signal for a salary-cap auction league.
