...
**Evidence:** `src/utils/projections.ts` now has a private `asArray()` applied to `playerScore`, `rosters.franchise`, and `franchise.player` (types widened to `T[] | T`). Regression suite: `tests/projections.test.ts` — includes the exact off-season fixture.

**Recommendation:** When writing utilities that consume raw MFL feed shapes, normalize every list field inside the utility with the triple-step pattern and widen the input types to `T[] | T`. Filter empties by field presence (`if (player.id && player.score)`) since the empty-object sentinel survives `asArray`.
