...
| Live scoring / ESPN data, game odds and weather | `docs/claude/rules/live-scoring.md` | A college athlete id and an NFL one are both plain digits, so a bad join resolves the wrong person instead of failing. And every league reads a game's spread, O/U and weather through ONE loader (`loadLiveOdds`, coach-data.ts): the AFL's own copy never filled weather, so the same game showed it in one league and not the other (`tests/nfl-odds-weather-guard.test.ts`). |
