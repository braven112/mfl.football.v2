...
| AFL waiver order (`waiverSortOrder`, `import?TYPE=franchises`) | `docs/claude/afl-rules.md` § Setting the waiver order | MFL drops waiver priority at every league-year rollover and the AFL is rolling-priority, so the default reverse-franchise-id order IS a live wrong waiver order; and the franchises import erases every field you don't send, so the write is invalid without `OVERLAY=1`. |
...
| League accounting, dues, prize payouts | `docs/claude/rules/accounting.md` | MFL credits on POSITIVE and its import has no delete — a prize written negative doubles the owner's bill, and a payout re-run without the idempotency check pays the whole season twice. |
