...
| Viewer preferences (country, clocks, `/preferences`, anything printing a kickoff time) | `docs/claude/rules/viewer-preferences.md` | Resolving the preference WRITES a cookie, so it belongs to the ROUTE — from a component `Astro.cookies.set()` throws after the headers are committed and blanks the page; and zone ids only mean anything against a country, so a country switch must re-default the clocks rather than carry the old ones. |
