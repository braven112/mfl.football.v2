...
| The broadcast board (`/broadcast`, reveals, red zone, the TV shell) | `docs/claude/insights/features/live-broadcast.md` | `toBroadcastPair` only DARKENS, so it cannot make a colour visible — seven TheLeague franchises are `#181818` and their full-screen reveal was a black rectangle on a black ground; and a projection belongs to a player IN A LEAGUE, so a cross-league board's shared player map cannot hold one (it held 0, which pinned every win-probability bar to a hard 100%). |
...
| Cron cadence, "the site is showing stale data", `vercel.json` crons | `docs/claude/rules/storage-and-build.md` § "GitHub's `schedule` is not a cadence" | GitHub DROPS this repo's scheduled events in bulk — `*/5` has delivered 5-8 runs a day since 2026-08-27, not 288 — and because the feeds are baked into the build, no sync run means no deploy and the page cannot move. The primary trigger is the Vercel cron in `vercel.json`; the workflow's own `schedule:` is a `*/30` fallback, and its cadence is a BUILD SPEND decision, since every sync commit to `main` is a production build. |
...
| The Board (`/suggestions`), the GitHub handoff, `/faq` | `docs/claude/rules/suggestion-box.md` | The board's Redis keys were bare while it was TheLeague-only, and two of them key on `franchiseId` ALONE — both leagues have a franchise 0001, so unscoped, an AFL owner reading the board marks a TheLeague owner's unread badge as read. Every board call takes its scope from the SESSION (`boardScope(user)`), never a param; and filing an idea as a GitHub issue is idempotent, because a 500 on a partial success invites the duplicate filing it was trying to prevent. |
