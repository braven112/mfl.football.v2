...
| Client-side data fetching (island `fetch`, inline-script `fetch`, polling, caching) | `docs/claude/rules/client-data.md` | There is ONE client data layer (`src/utils/query-store.ts`) and it is not a library — a provider-based cache would be one cache per island, because Astro gives every island its own React root; and a failed request must never be cached as an empty result or a logged-out user. |
