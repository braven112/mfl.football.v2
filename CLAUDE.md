...
| Client-side data fetching, or writing/moving an `is:inline` script | `docs/claude/rules/client-data.md` | There is ONE client data layer (`src/utils/query-store.ts`) and it is not a library — a provider-based cache would be one cache per island, because Astro gives every island its own React root; a failed request must never be cached as an empty result or a logged-out user; and inline script is ratcheted DOWN (`tests/inline-script-ratchet.test.ts`), with the catch that neither an inline script nor a module re-runs on a ClientRouter return visit — wire on `astro:page-load`. |
