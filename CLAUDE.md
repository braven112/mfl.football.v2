...
| Franchise history, owner attribution, `ownerHistory`, owner pages | `docs/claude/insights/features/franchise-history.md` + `docs/plans/owners-feature.md` | Owner-scoping drops a third of all franchise-seasons off franchise pages; `/owners` is now where they live, so every such season must land on exactly one owner or it vanishes again — `tests/owner-tenures-data.test.ts` pins that. Four copies of the ownership-boundary rule remain and two already disagree. |
