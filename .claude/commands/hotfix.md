...
**8b. Open the follow-up issue.** One issue per brief — the items are a checklist
inside it, not separate issues. It is the durable copy: a session transcript
expires, a working tree is thrown away, and the brief itself can fail to push.
The issue survives all three, so **write the items into the body rather than
linking to the brief.** The duplication is the point.

```bash
gh label create hotfix-followup --color 0E8A16 --description "Deferred work from a hotfix" 2>/dev/null || true
gh issue create --label hotfix-followup \
  --title "Follow-up: <slug>" \
  --body "<the brief's What broke / What the hotfix did / Deferred items / Context sections>"
```

In a cloud session use `mcp__github__issue_write` with `method: "create"`.

The body must carry, at minimum: the hotfix PR URL and squash SHA, and every
deferred item as a `- [ ]` checkbox with its `file:line`, its source, and why it
was deferred. End it with the attribution footer:
...
Then comment the issue link onto the merged hotfix PR, so the two halves are
reachable from either end.

**8c. Push the brief.** Add the issue URL to the brief's `followup_issue` field
first, then commit and push. A new cloud session clones the repo fresh at
container start, so a brief that exists only in this session's working tree does
not exist for the child. This step fails silently on both sides, so verify the
push landed rather than assuming it:
...
If this push fails and you can't fix it quickly, **do not block** — 8b already
captured the work, and 8d hands over the issue number too. Say so in the report.

**8d. Start the follow-up session.** Cloud sessions are the normal case, so this
...
mcp__Claude_Code_Remote__create_session
  ...
  prompt: "/followup docs/claude/followups/<YYYY-MM-DD>-<slug>.md #<issue>"
```

Pass **both** the brief path and the issue number. They are the same content by
design: if the 8c push didn't land, the child reads the issue instead of stopping.

Three rules keep it cheap:

- **Pass a pointer, not a payload.** The prompt is two references and nothing
  else. Do not paste the findings, the diff, or a recap of the incident into it —
  both copies are already reachable, and anything inlined is paid for twice.
...
Issue         #<n> opened (hotfix-followup)
Follow-up     session <id> started / run `/followup <path> #<n>`
