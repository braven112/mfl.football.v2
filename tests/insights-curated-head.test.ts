...
const CURATED = ['frontend.md', 'design-system.md', 'mfl-api.md'];
...
function readDomain(file: string): string {
  const path = join(ROOT, DOMAINS, file);
  expect(existsSync(path), `${DOMAINS}/${file} is missing`).toBe(true);
...
describe('insights curated heads', () => {
  it.each(CURATED)('%s has a curated head within the size budget', (file) => {
    const head = extractHead(readDomain(file), file);
    const bytes = Buffer.byteLength(head, 'utf8');

    expect(
      bytes,
      `${DOMAINS}/${file}'s curated head is ${bytes} bytes (max ${MAX_HEAD_BYTES}). ` +
    ...
    expect(
      bytes,
      `${DOMAINS}/${file}'s curated head is only ${bytes} bytes — that is a stub, not ` +
  ...
  it.each(CURATED)('%s tells the reader not to read the archive', (file) => {
    ...
    expect(
      /grep/i.test(head),
      `${DOMAINS}/${file}'s head must tell the reader to grep the archive rather than ` +
  ...
  it('the archive still carries dated entries below the head', () => {
    ...
    for (const file of CURATED) {
      const body = readDomain(file);
      const archive = body.slice(body.indexOf(CLOSE) + CLOSE.length);
      const dated = archive.match(/^## .*\d{4}-\d{2}-\d{2}/gm) ?? [];

      expect(
        dated.length,
        `${DOMAINS}/${file} has ${dated.length} dated entries left below the head. ` +
  ...
  it('every domain file is either curated or small enough to read whole', () => {
    ...
    for (const file of readdirSync(join(ROOT, DOMAINS))) {
      if (!file.endsWith('.md') || CURATED.includes(file)) continue;
      const bytes = Buffer.byteLength(readDomain(file), 'utf8');
      if (bytes > 64 * 1024) oversized.push(`${file} (${Math.round(bytes / 1024)} KB)`);
    }
