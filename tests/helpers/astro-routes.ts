 ...
 * looked for a literal file would call every article link broken the moment
 * Schefter refers back to a previous column — which the new "link everything"
 * directive makes likely. A guard that cries wolf on real links is one nobody
 * keeps. A segment therefore matches:
...
export function astroRouteExists(href: string, pagesDir = PAGES_DIR): boolean {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // this whole check a silent no-op that still reports green.
