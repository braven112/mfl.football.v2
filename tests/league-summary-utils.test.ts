...
describe('computeLeagueSummary draft capital', () => {
  it('should return 3 picks for years 0 and 2-4 (standard allocation)', () => {
    const teams = [makeTeam()];
    const result = computeLeagueSummary([], [], null, teams);
    const team = result[0];

    expect(team.metrics.draftCapital[0]).toBe(3);
  ...
  it('should use actual pick count from futureDraftPicks for year 1 (next draft)', () => {
    const draftYear = SALARY_YEARS[0] + 1;
    ...
    expect(team.metrics.draftCapital[1]).toBe(4); // has 4 picks for next draft
  });
});
